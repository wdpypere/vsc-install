...
class CommontestTest(TestCase):
    ...
    def test_check_autogenerated_ci_config_file(self):
        ...
        expected_error_msg += f"you should run 'python -m vsc.install.ci' again to re-generate {test_fn}"

        # check using existing test file => assertEqual should be called with found + expected contents
        Path(test_fn).write_text('foobar', encoding='utf8')
        ...
        Path(test_fn).write_text(test_contents, encoding='utf8')
        check_autogenerated_ci_config_file(mocked_self, test_fn, test_contents)
        mocked_self.assertTrue.assert_called_once_with(True)  # file existence check
        ...
        expected_error_msg = f"Found {test_fn}.NOT_AUTOGENERATED_YET, so contents of {test_fn} is expected "
        ...
        Path(test_fn + '.NOT_AUTOGENERATED_YET').write_text('', encoding='utf8')
        check_ignore(False)

        Path(test_fn + '.NOT_AUTOGENERATED_YET').write_text('https://github.com/hpcugent/vsc-install/issues/1234', encoding='utf8')
        check_ignore(True)
